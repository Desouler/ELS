...
- Copy .env.example to .env
  ...
  cp .env.example .env
  ```
- Change passwords inside .env and save

- Run tls service with docker compose
  ```sh
  docker compose up tls
  ```

- Run setup service with docker compose
...
Sometimes elastic does not finish starting and filebeat fails (although it depends on elastic, probably health check is needed), so you may need to restart it
```sh
  docker compose restart filebeat
  ```

You can access my app on http://localhost:33333. You will need to create an API key from security menu inside kibana for use in my app.
